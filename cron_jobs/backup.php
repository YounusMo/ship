...
function run(){
    ...
    if($fileCount > 40){
        ...
        $sourceDir = __DIR__ . '/backups'; // مسار الملفات يلي بدك تضيفها للـ ZIP
        $zipFile = __DIR__ . '/backups_zip/'.date('Y-m-d H:i:s').'.zip'; // اسم ملف الـ ZIP الناتج

        $zip = new ZipArchive();

        if ($zip->open($zipFile, ZipArchive::CREATE | ZipArchive::OVERWRITE)) {
            $files = glob($sourceDir . '/*.sql'); // جلب كل الملفات بصيغة .sql

            foreach ($files as $file) {
                $zip->addFile($file, basename($file)); // إضافة الملف مع اسمه فقط داخل ZIP
            }
    ...
    $tables = [
      ...
      "users"
    ];// Add specific tables in this array if needed
    ...
    function Export_Database($host, $user, $pass, $name, $tables = false, $backup_name = false) {
        ...
        try {
            ...
            $file_name = __DIR__.'/backups/' . date('Y-m-d') . ' ' . date('H:i') . '.sql';
