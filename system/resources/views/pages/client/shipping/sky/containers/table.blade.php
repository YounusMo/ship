...
@foreach ($get as $item)
    @if (in_array($item->id , $ids))
        <div class="card mb-3" style="width: 95%">
            <div class="card-body">
                <div class="mb-2"><strong>{{$lang->write('Trip name')}} : </strong> {{$item->name}}</div>
                <div class="mb-2"><strong>{{$lang->write('Trip number')}} :</strong> {{$item->number}}</div>
                ...
                <div class="mb-2">
                    @if ($item->type === 'full')
                        <a class="btn btn-primary w-100 mt-2" href="{{url('/client/shipping/sky/container')}}/{{$item->id}}" target='_blank'>{{$lang->write('Show')}}</a>    
                    @endif
