        ...
        @foreach ($get as $item)
            @php
                ...
                if (in_array(auth()->user()->type , ['admin','office_work',])) {
                    ...
                    $chk_clients = DB::table('store_out_sea')->where('container_id',$item->id)->get();
                ...
                // $chk_money_3 = DB::table('suppliers_transactions')->where('sky_sea','sea')->where('container_id',$item->id)->count();
                // $chk_money_4 = DB::table('customs_brokers_transactions')->where('sky_sea','sea')->where('container_id',$item->id)->count();

                // if($chk_money_1 > 0 ||$chk_money_2 > 0 ||$chk_money_3 > 0 ||$chk_money_4 > 0 ){
                //     $allow_cancel = false;
                // }

                if($item->custom_status === 'pending'){
                    $allow_cancel = false;
                }
            @endphp
            @if ($show)
                <tr>
                    ...
                    <td>
                        <select class="form-select status" {{in_array(auth()->user()->type , ['admin','branch_admin']) ? '' : 'disabled'}} data-id="{{$item->id}}">
                    ...
                    <td>
                        ...
                        @if (in_array(auth()->user()->type , ['admin','branch_admin'])) 
                            @if ($item->canceled === 'false')
                                ...
                                @if ($item->custom_status === 'approved')
                                    <button class="btn btn-success btn-sm" onclick="showCustoms({{$item->id}})">{{$lang->write('Customs clearance')}}</button>    
                                @endif
                                ...
                                @if ($item->type === 'custom')
                                    @if ($item->custom_status === 'pending')
                                        <button class="btn btn-success btn-sm" onclick="upadteStatusCustomContainer({{$item->id}} , 'approved')">{{$lang->write('Approve')}}</button>
                                        <button class="btn btn-danger btn-sm" onclick="upadteStatusCustomContainer({{$item->id}} , 'rejected')">{{$lang->write('Reject')}}</button>
                                    @endif
                                    <button class="btn btn-primary btn-sm" onclick="showContainer({{$item->id}})">{{$lang->write('Show')}}</button>    
                                @endif
