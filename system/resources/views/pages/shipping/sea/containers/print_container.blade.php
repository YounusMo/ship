...
<div class="print-wrap" style="direction: {{ $isRtl ? 'rtl' : 'ltr' }}; color: #1a2233; font-family: 'Helvetica Neue', Arial, sans-serif; padding: 18px; font-size: 12px;">
    ...
    @include('partials.print_header', [
        'settings' => $settings,
        'lang'     => $lang,
        'title'    => $lang->write('Sea container') . ' — ' . $get->name,
        'subtitle' => '#' . $get->number,
    ])
    ...
    <table style="width: 100%; border-collapse: collapse; margin-bottom: 16px;">
        <thead>
            <tr>
                ...
                <th style="{{ $cellTh }}">{{ $lang->write('Container size') }}</th>
                <th style="{{ $cellTh }}">{{ $lang->write('Shipping line') }}</th>
        ...
        <tbody>
            <tr>
                ...
                <td style="{{ $cellTd }}">{{ $supplier->name ?? '-' }}</td>
