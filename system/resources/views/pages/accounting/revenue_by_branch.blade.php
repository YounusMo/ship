...
<div class="page-header">
    <div>
        <h1 class="page-title">{{ $lang->write('Revenue by branch') }}</h1>
        <div class="page-subtitle">
            {{ $lang->write('Revenue grouped by branch_id from journal_lines') }} ·
...
<div class="card">
    <div class="card-body">
        @if(empty($branches))
            <div class="alert alert-info">{{ $lang->write('No revenue in this period.') }}</div>
        @else
                    ...
                    @foreach($branches as $br)
                        <tr>
                            <td>
                                {{ $br['name'] }}
                                @if(empty($br['id']))
                                    <span class="badge bg-warning text-dark" title="{{ $lang->write('Revenue without branch_id — fix at source per the cardinal rule') }}">
