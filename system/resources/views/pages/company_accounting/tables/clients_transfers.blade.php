...
<table class="table" style="width: 100%">
    <thead>
        <tr>
            ...
            <th style="{{$th_style}}">{{$lang->write('From currency')}}</th>
            <th style="{{$th_style}}">{{$lang->write('To currency')}}</th>
    ...
    <tbody>
        @foreach ($get as $item)
            ...
            <tr>
                ...
                <td style="{{$td_style}}">{{$dataController->numberFormat($item->value)}} {{$cur}}</td>
                <td style="{{$td_style}}">{{$dataController->numberFormat($item->transfer_value)}} {{$cur2}}</td>
                <td style="{{$td_style}}">{{round($item->exchange_rate, 3)}}</td>
                <td style="{{$td_style}}">{{strlen($item->notes) > 0 ? $item->notes : '-'}}</td>
