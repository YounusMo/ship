    ...
    <tbody>
        @foreach ($get as $item)
            ...
            <tr>
                ...
                {{-- <td>{{$dataController->get_type($item->type)}}</td> --}}
                ...
                <td style="{{$td_style}}">
                    @if ($data->purpose === 'salary')
                        {{$lang->write(ucwords($data->purpose))}} / {{$users[$data->users] }}
                    @else
                        {{$lang->write(ucwords(str_replace(['_'] , ' ',$data->purpose)))}}    
                    @endif
                </td>
